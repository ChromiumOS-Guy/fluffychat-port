...
/bin/cp $ROOT/assets/fluffychat-logo.png $BUILD_DIR 2>/dev/null >> $LOG_FILE # copy fluffychat logo into click package

/bin/sed -i "s/@CLICK_ARCH@/${ARCH_TRIPLET}/" ${BUILD_DIR}/fluffychat-launch.sh >> $LOG_FILE # change arch for launch script

/bin/cp -rn $BUILD_DIR/usr/lib/* $INSTALL_DIR/lib 2>/dev/null >> $LOG_FILE # copy maliit libs to install dir

/bin/chmod -R +x $INSTALL_DIR/lib/$ARCH_TRIPLET/* # make sure there is no permission problem for libs (we make sure now the critical libs are covered anyways)
#/bin/chmod -R +x $INSTALL_DIR/lib/* (we run this in fluffychat-d.sh so it also covers fluffychat libs)
...
if ! $BUILD_DIR/scripts/fluffychat-d.sh >> $LOG_FILE; then # move app files to build
  ...
  echo "Successfully downloaded fluffychat binaries!" >> $LOG_FILE
fi
